...
public class ManejadorDocente {
    ...
    public List<Docente> ListarDocentesCursoAsignaturaAnio(int AnioCurso, int IdAsignatura){
        List<Docente> lista = new ArrayList<>();
        try{
            TypedQuery<Docente> query = em.createQuery("SELECT d FROM Docente d, Curso c WHERE c.AnioCurso= :anioCurso and c.AsignaturaCurso.IdAsignatura= :idAsignatura", Docente.class);
            query.setParameter("anioCurso", AnioCurso);
    ...
    public List<Docente> ListarDocentesCursoAsignaturaSemestre(int SemestreCurso, int IdAsignatura){
        List<Docente> lista = new ArrayList<>();
        try{
            TypedQuery<Docente> query = em.createQuery("SELECT d FROM Docente d, Curso c WHERE c.SemestreCurso= :semestreCurso and c.AsignaturaCurso.IdAsignatura= :idAsignatura", Docente.class);
            query.setParameter("semestreCurso", SemestreCurso);
            query.setParameter("idAsignatura", IdAsignatura);
            lista = query.getResultList();
        }catch(Exception ex){
    ...
    public List<Docente> ListarDocentesCursoAsignatura(int IdAsignatura){
        List<Docente> lista = new ArrayList<>();
        try{
            TypedQuery<Docente> query = em.createQuery("SELECT d FROM Docente d, Curso c WHERE c.AsignaturaCurso.IdAsignatura= :idAsignatura", Docente.class);
            query.setParameter("idAsignatura", IdAsignatura);
            lista = query.getResultList();
        }catch(Exception ex){
            System.out.println("Error: " + ex.getMessage());
